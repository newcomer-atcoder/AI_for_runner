...
class AIFacade:
    ...
    def addTrain(
        self,
        dist_cond_orig : list[list[float]],
        runnningDists_orig : list[list[float]]
    ):
        distance_conditions = torch.tensor(dist_cond_orig, dtype=torch.float32)
        runnningDists = torch.tensor(runnningDists_orig, dtype=torch.float32)
